...
const CATEGORIES_QUERY = `*[_type == "category"] | order(name asc){ name, slug, description, image }`;
...
const HERO_BANNERS_QUERY = `*[_type == "heroBanner"] | order(order asc){ _id, alt, link, image }`;
...
export async function getHeroBanners(): Promise<HeroBanner[]> {
  const raw = await sanityClient.fetch<
    { _id: string; alt: string; link: string | null; image: RawImage }[]
